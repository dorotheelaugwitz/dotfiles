#!/bin/sh
# Open text/code files in VSCode (instead of Xcode) when clicked in the terminal
...
for uti in public.plain-text public.source-code public.script \
    ...
    duti -s "$app" "$uti" all 2>/dev/null
done

echo "==> Set VSCode as the default app for text/code files"
